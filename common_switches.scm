...
(define help_switch_handler
  (lambda ()
    ...
    (newline)
    (display "dtcalc -d dd.mm.yyyy (+/add)/(-/sub) num")
    (newline)
    ...
    (newline)
    (display "how to calculate time:")
    (newline)
    (display "dtcalc -t hh.mm.ss (+/add)/(-/sub) num")
    (newline)
    (display "using -t/--time without any options will show current time")
    (newline)
    (display "dtcalc -t 12.24.56 + 7 => 12.25.3")
    (newline)
    (display "dtcalc -d 12.24.56 - 150 => 12.22.26")
    (newline)
    (exit)))
